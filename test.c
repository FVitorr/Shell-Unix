        ...
        if (tpipe >= 1){//TEM PIPE
            ...
            while (ok == 0) {
                ...
                if(first == 0){
                    ...
                    pidb = do_fork(pipefd, PONTA_LEITURA, STDIN_FILENO, args_pipe);

                    for(int i=0; i < tam_pipe; i++){
                    printf("argsPipe2[%d] = %s\n", i, args_pipe[i]);
                }
                    ...
                    if(verificaPipe(args, tam_arg, init) > 0 && first != 0) {
                ...
                tam_pipe = 0;
                argsPipe(args, tam_arg, args_pipe, &tam_pipe, init); //Encontrar Primeiro Argumento
                args_pipe[tam_pipe] = NULL;
                tam_pipe++;

                // aqui começa caso haja mais de um pipe |
                if (verificaPipe(args, tam_arg, init-1) > 0) {
                    printf("ENTROU\n");
                    int pipefd1[2];
                    criaPipe(pipefd1);
                    ...
                    // ========== FILHO A ==========
                    // Alterar o descriptor de SAIDA padrão (Monitor) para a ponta de LEITURA do PIPE - processo a direita
                    do_fork(pipefd1, PONTA_LEITURA, STDIN_FILENO, args_pipe);
                    do_fork(pipefd, PONTA_ESCRITA, STDOUT_FILENO, args_pipe);

                    for(int i=0; i < tam_pipe; i++){
                    printf("argsPipe3[%d] = %s\n", i, args_pipe[i]);

                                            //Encerrar DUP2
                    close(pipefd1[1]);
                    close(pipefd1[0]);//0 leitura
                                        //Encerrar DUP2
                    close(pipefd[1]);
                    close(pipefd[0]);//0 leitura
                }
                    ...
                    if (verificaPipe(args, tam_arg, init) > 0) {
                        printf("ENTROU NESSEEEE\n");
                        // ========== FILHO B ==========
                        // Alterar o descriptor de ENTRADA padrão (Teclado) para a ponta de LEITURA do PIPE - processo a direita
                        pidb = do_fork(pipefd1, PONTA_LEITURA, STDIN_FILENO, args_pipe);
                        //Encerrar DUP2
                        close(pipefd1[1]);
                        close(pipefd1[0]);//0 leitura
                    ...
                    //pidb = waitpid(pidb, NULL, 0); // Aguardar o ultimo processo ser finalizado
                    }
                } else {
                    //pidb = waitpid(pidb, NULL, 0); // Aguardar o ultimo processo ser finalizado
                    ok = 1;
